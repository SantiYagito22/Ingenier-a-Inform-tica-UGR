...
Esfera::Esfera(const int num_vert_perfil, const int num_instancias_perfil, const float radio,char rotacion,bool text) {
    std::vector<Tupla3f> perfilOriginal;

    for(int i=1; i<num_vert_perfil;i++)
    {

        perfilOriginal.push_back(Tupla3f{radio*cos(((PI*i)/num_vert_perfil)+ 3*PI/2),radio*sin(((PI*i)/num_vert_perfil)+ 3*PI/2),0.0});
