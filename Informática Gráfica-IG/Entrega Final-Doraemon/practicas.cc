...
void clickRaton(int boton, int estado,int x,int y){
    if(boton == GLUT_RIGHT_BUTTON ) {
        if (estado == GLUT_DOWN) {
            escena->estadoRaton = true;
            escena->xant = x;
            escena->yant = y;
        } else
            escena->estadoRaton = false;
...
int main( int argc, char **argv )
{
   ...
   glutCreateWindow("Practicas IG: Santiago Munoz Castro");
