...
public class Corellian extends LARVAFirstAgent{
     ...
     * @param x la posición x a la que tiene que ir
     * @param y la posición y a la que tiene que ir
    ...
    */
    ...
    public boolean needRecharge(int altitude, double energy){
        ...
        if(altitude == 0)
            return (energy <= (10 + mySensors.length));
        else
            return (energy <= ((altitude/5) * 10)+ mySensors.length+10);
    ...
    public void moveTo( int coord, int lidar[][], int compass){
        ...
        if(cont>=4)
        {
            while(compass != coord){
                act("RIGHT");
                if((compass-45)<0){
                   compass = 360+(compass-45);
                }
                else{
                    compass=(compass-45) % 360;
                }
            }
        }
        else
        ...
        while(lidar[coord_lidar[0]][coord_lidar[1]] < 0){
            act("UP");
            alturaRecargar += 5;
            lidar[coord_lidar[0]][coord_lidar[1]] += 5;
  
        }
        
        if(lidar[coord_lidar[0]][coord_lidar[1]] >= 0)
        {
            act("MOVE");
        }
    ...
    * @author Amanda Moyano Romero
    ...
    public void recargar(){
        ...
        if (needRecharge(alturaRecargar, energy)) {
            ...
            destroyer.setInReplyTo("CAPTURE " + xIr + " " + yIr + " " + zIr);
    ...
    */
    ...
    public void capturarJedi(int x, int y, int z)
    {
        updateSensors();
        int altitude = myDashboard.getLidar()[5][5];
    ...
    */
    ...
    public boolean recibirCaptura(){
        ...
        Info("Esperando a recibir");
        open = this.LARVAblockingReceive(t);
        
        if(open.getPerformative() == ACLMessage.REQUEST){
           
           String parse[] = open.getContent().split(" ");
           xIr= Integer.parseInt(parse[1]);
           yIr= Integer.parseInt(parse[2]);
           zIr = Integer.parseInt(parse[3]);
           ...
           inicializarVectorLidar();
            ...
            while(myDashboard.getDistance() > 0 && myDashboard.getGPS()[2] >= 0 ){  
                
                //obtenemos la orientación a la que se tiene que mover
                int next = nextStep(xIr, yIr, myDashboard.getGPS(), myDashboard.getLidar());
